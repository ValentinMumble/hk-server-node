...
app.get('/spotify/addok/:uri', (req, res) => {
  spotifyApi.removeTracksFromPlaylist(process.env.SPO_OK_ID, [{ uri: req.params.uri }]).then(
    () => spotifyApi.addTracksToPlaylist(process.env.SPO_OK_ID, [req.params.uri]).then(
...
app.get('/hue/off', (req, res) => {
  bobApi.setGroupLightState(0, lightState.create().turnOff())
